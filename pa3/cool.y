...
%{
  ...
  for the location of tokens */
    ...
    */
	...
	| feature_list feature ';' {/*multi-features*/
		$$ =  append_Features($1, single_Features($2) );
	}
	| feature_list error ';'/*error*/ {;}

    feature:
	OBJECTID '(' ')' ':' TYPEID '{' expr '}'
	{ $$ = method($1, nil_Formals(), $6, $8); }
	| OBJECTID '(' formal_list ')' ':' TYPEID '{' expr '}'
	{ $$ = method($1, $3, $6, $8); }
	| OBJECTID ':' TYPEID { $$ = attr($1, $3, no_expr()); }
	| OBJECTID ':' TYPEID ASSIGN expr { 
		$$ = attr($1, $3, single_Expression($5) ); }
    
    formal_list:
	formal { $$ = single_Formals($1); }
	| formal_list ',' formal { $$ = append_Formals($1, $3); }
	...
	OBJECTID ':' TYPEID ASSIGN expr IN epxr { $$ = let($1, $3, $5, $7);}
	OBJECTID ':' TYPEID ',' expr_let { $$ = let($1, $3, no_expr(), $5); }
	OBJECTID ':' TYPEID ASSIGN expr ',' expr_let {$$ = let($1, $3, $5, $7); }
    ...
    expr:
	OBJECTID ASSIGN expr { $$ = assign($1, single_Expressions($3)); }
	| expr '@' TYPEID '.' OBJECTID '(' ')'
		{ $$ = static_dispatch($1, $3, $5, nil_Expression()); }
	| expr '@' TYPEID '.' OBJECTID '(' expr_list_comma ')'
		{ $$ = static_dispatch($1, $3, $5, $7); }
	| expr '.' OBJECTID '(' ')'
		{ $$ = dispatch($1, $3, nil_Expression()); }
	| expr '.' OBJECTID '(' expr_list_comma ')'
		{ $$ = dispatch($1, $3, $5); }
	| OBJECTID '(' ')' { 
		$$ = dispatch(object(single_Expression(self)), $1, nil_Expression()); }
	| OBJECTID '(' expr_list_comma ')' { 
		$$ = dispatch(object(single_Expression(self)), $1, single_Expressions($3)); }
	| IF expr THEN expr ELSE expr FI {$$ = cond($2, $4, $6);}
	| WHILE expr LOOP expr POOL { $$ = loop($2, $4); }
	| '{' expr_list_semicolon '}' { $$ = block($2); }
	| LET expr_let { $$ = $2; } 
	| CASE expr OF case_list ESAC {$$ = typecase($2, $4);}
	| NEW TYPEID { $$ = new_($2); }
